...
class evk02001_init_tx(gr.sync_block):
    """
    ...
    def __init__(self, trx_frequency, initial_tx_beam_index, initial_rx_beam_index, initial_TX_RF_gain, initial_TX_BF_gain, initial_RX_RF_gain, initial_RX_vga_1_2_gain, unit_name, sleep_amount, evkplatform_type="MB1", version_num="20211123_1800", custom_beambook_suffix="", do_lo_leakage_cal=False, ref_freq = 122.88e6, ref_is_diff=True, rfm_type='BFM02003'):
        ...
        if hasattr(self.CRUIJFF, 'regs') and self.TRX_accessible(): #check whether regs are already initialized and if self.EDER.chip_present_status == True
            ...
            # print("[EVK02001 Init TX]: TX setup with custom beambook suffix: " + self.custom_beambook_suffix)
            # Reset antenna array
            self.CRUIJFF.reset()
            ...
            self.CRUIJFF.runTx(trx_frequency)
            self.CRUIJFF.regs.wrrd('trx_tx_on', 0x1fffff) # activate all RF paths
            self.set_tx_default_gain()
            #self.CRUIJFF.tx.set_beam(0x0a)
            ...
            if sleep_amount > 0:
                print("You can now connect the SPI connector and use direct SPI control. The following SPI read error is generated by the USB FTDI chip which not cannot communicate over SPI anymore. This is intended.")
            ...
            print('[EVK02001 Init TX]: ERROR: Init failed (Chip not present)')
        return
        ...
        with self.thread_lock:
          ...
          #Currently only RX_beam_ID message to set RX_beam
            ...
            if pmt.is_dict(meta) == False:
                print("[EVK02001 Init TX]: ERROR: Recevied MAC message meta is not a PMT dictionary!")
            ...
            if not(rx_beam < 64 and rx_beam >= 0):
                print('[EVK02001 Init TX]: ERROR: Received RX beam ID is out of range!')
    ...
    def set_tx_default_gain(self):
        if self.CRUIJFF.chip_is_present():
            ...
            self.CRUIJFF.regs.wr('tx_bb_gain', 0x01) #old: 0x0
            self.CRUIJFF.regs.wr('tx_bb_iq_gain', 0xff) #old0x00
            print("[EVK02001 Init TX]: TX default gains set")
        else:
            print("[EVK02001 Init TX]: ERROR: Cannot set TX default measurement gains, chip not present or mode not TX!")
    ...
    def set_rx_default_gain(self):
        if self.CRUIJFF.chip_is_present():
            ...
            print("[EVK02001 Init TX]: RX default gains set")
        else:
            print("[EVK02001 Init TX]: ERROR: Cannot set RX default measurement gains, chip not present or mode not TX!")
